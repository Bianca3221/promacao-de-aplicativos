open('planejamentoviagem.txt', 'w').close()

def criar_destino():
    destino = input("Coloque o destino desejado:")
    with open('planejamentoviagem.txt', 'a') as f:
        f.write(destino + '\n')
        print("Destino adicionado! ")

def ler():
    with open('planejamentoviagem.txt', 'r') as f:
        lugares = f.readlines()
...
def atualizar():
    ...
    with open('planejamentoviagem.txt', 'w') as f:
        f.writelines(linhas)
